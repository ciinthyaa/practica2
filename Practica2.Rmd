...
5. Para cada enlace, seguirlo e indicar si está activo (podemos usar el código de status HTTP al hacer una petición a esa URL).
    ...
    Se buscan las coincidencias y si no coincide se pega la URL faltante:
...
``` 
    ...
    Se crea la columna según sea la función:
    
```{r}
    tabla$linksr <- sapply (tabla$URL, asdf)
``` 
  
    Verificar el estado de cada link:
...
1. Un histograma con la frecuencia de aparición de los enlaces, pero separado por URLs absolutas (con “http…”) y URLs relativas.
  ...
  Creamos una nueva columna llamada 'Tipo' para distinguir entre URLs absolutas y relativas
```{r}
tabla$Tipo <- ifelse(grepl("^http", tabla$URL), "URL Absoluta", "URL Relativa")
``` 

Creamos el histograma

```{r}
ggplot(data = tabla, aes(x = Tipo, fill = Tipo)) + geom_bar() + labs(title = "Histograma de Enlaces por Tipo", x = "Tipo de Enlace", y = "Frecuencia") + theme_minimal()
``` 
...
Creamos una nueva columna llamada 'TipoEnlace' para guardar la diferencia entre enlaces a mediawiki y otros:

```{r}
 tabla$TipoEnlace <- ifelse(
     grepl("https://www.mediawiki.org", tabla$URL),
     "Enlace a mediawiki",
     "Otros Enlaces"
 )
``` 
...
```{r}
 ggplot(data = tabla, aes(x = TipoEnlace, fill = TipoEnlace)) +
     geom_bar() +
     labs(title = "Enlaces a mediawiki vs. Otros Enlaces", x = "Tipo de Enlace", y = "Suma de Enlaces") + theme_minimal()
``` 
3. Un gráfico de tarta (pie chart) indicando los porcentajes de Status de nuestro análisis.

```{r}
 ggplot(data = tabla, aes(x = "", y = estado, fill = estado)) +
    geom_bar(stat = "identity", width = 1, color = "white") +
     coord_polar("y") +
     labs(title = "Distribución de Estado de Análisis", fill = "estado") +
     theme_minimal() +
     theme(legend.position = "right")
``` 

Se deben guardar los 3 graficos en variables:

```{r}
Gra1 <- ggplot(data = tabla, aes(x = Tipo, fill = Tipo)) + geom_bar() + labs(title = "Histograma de Enlaces por Tipo", x = "Tipo de Enlace", y = "Frecuencia") + theme_minimal()
```

```{r}
Gra2 <- ggplot(data = tabla, aes(x = TipoEnlace, fill = TipoEnlace)) +
     geom_bar() +
     labs(title = "Enlaces a mediawiki vs. Otros Enlaces", x = "Tipo de Enlace", y = "Suma de Enlaces") + theme_minimal()
```

```{r}
Gra3 <-ggplot(data = tabla, aes(x = "", y = estado, fill = estado)) +
    geom_bar(stat = "identity", width = 1, color = "white") +
     coord_polar("y") +
     labs(title = "Distribución de Estado de Análisis", fill = "estado") +
     theme_minimal() +
